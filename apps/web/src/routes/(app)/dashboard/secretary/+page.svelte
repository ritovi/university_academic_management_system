<script lang="ts">
	...
	import { secretaryNavGroups } from '$lib/components/config/secretaryNav';
	let { children } = $props();
	...
	const activeParts = $derived.by(() => {
		...
		for (const group of secretaryNavGroups) {
...
<DashboardLayout>
	{#snippet sidebar()}
		<AppSidebar navGroups={secretaryNavGroups} />
	{/snippet}
	...
	{#snippet headerContent()}
		<Breadcrumb.Root>
			<Breadcrumb.List>
				<Breadcrumb.Item>
					<Breadcrumb.Link href="/dashboard/secretary">Secretaría</Breadcrumb.Link>
				</Breadcrumb.Item>
