<script lang="ts">
	...
	import { adminNavGroups } from '$lib/components/config/adminNav';
	...
	const activeParts = $derived.by(() => {
		...
		if (activeItem && currentPath !== activeItem.url) {
			if (currentPath.startsWith(`${activeItem.url}/new`)) {
				parts.push({ title: 'Crear Nuevo' });
			}
		}
...
<DashboardLayout>
	...
	{#snippet headerContent()}
		<Breadcrumb.Root>
			<Breadcrumb.List>
				<Breadcrumb.Item>
					<Breadcrumb.Link href="/dashboard/admin">Admin</Breadcrumb.Link>
				</Breadcrumb.Item>
